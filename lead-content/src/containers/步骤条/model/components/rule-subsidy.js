...
import {
  Form, Input, Select, Radio, Button, InputNumber, message,
} from 'antd';
import './index.less';
...
export default class Rule extends React.Component {
  constructor(props) {
    ...
    if (props?.ruleDetail?.indicatorValueType == '2') {
      ...
      props?.ruleDetail?.ruleDetail?.ladders?.map((val, index) => {
        const flot = props?.ruleDetail?.ruleDetail?.ladders[index]?.startPoint;
    ...
    if (props?.ruleDetail?.indicatorValueType == '4') {
      ...
      props?.ruleDetail?.ruleDetail?.ladders?.map((val, index) => {
        ...
        val?.nextNode?.ladders?.map((item, itemdx) => {
          const flotg = props.ruleDetail.ruleDetail.ladders[index].nextNode.ladders[itemdx].startPoint;
          if (flotg == true || flotg == 'true') {
  ...
  render() {
    const {
      data, step5Json, carDrived, orderDataDrived,
    } = this.state;
    const {
      type,
    } = this.props;
    ...
    return (
      <Form layout="inline">
        <div>
          <FormItem labelCol={{ style: { width: 130 } }} label="负激励名称:">
            {getFieldDecorator('name', {
              rules: [{
                required: true, message: '请输入',
              }],
              initialValue: step5Json?.name,
            })(
              <Input style={{ width: 160 }} maxLength={15} disabled={type == 'detail'} />,
            )}
          </FormItem>
        </div>
        <FormItem labelCol={{ style: { width: 130 } }} label="统计对象:">
          {getFieldDecorator('statisticObject', {
            rules: [{
              required: true, message: '请输入',
            }],
            initialValue: step5Json?.statisticObject?.toString(),
          })(
        ...
        <div>
          <FormItem labelCol={{ style: { width: 130 } }} label="模型维度:">
            {getFieldDecorator('indicatorDimension', {
              rules: [{
                required: true, message: '请输入',
              }],
              initialValue: step5Json?.indicatorDimension?.toString() || '1',
            })(
            ...
            getFieldValue('indicatorDimension') == 1 ? (
              <FormItem labelCol={{ style: { width: 130 } }} label="数据类型:">
                {getFieldDecorator('indicatorValueType', {
                  ...
                  initialValue: step5Json?.indicatorValueType?.toString(),
                })(
        ...
        <div>
          {
            getFieldValue('indicatorValueType') == '2' && (
              <table className="table-all" style={{ marginLeft: 45, marginTop: 20 }}>
                          ...
                          <FormItem>
                            {
                              getFieldDecorator(`ruleDetail.ladders[${ind}].ladderValue`, {
                                ...
                                <Input style={{ width: 120, margin: 0 }} placeholder="请输入" />,
                              )
                            }
                          </FormItem>
        ...
        <div>
          {
            getFieldValue('indicatorValueType') == '1' && (
              <table border="1" className="table-all" style={{ marginLeft: 45, marginTop: 20 }} cellSpacing="0">
                <thead className="table-con">
                  <tr>
                    <th width="280">参数1</th>
                    <th width="280">区间（≤X{'<'}）</th>
                    <th width="120">扣减金额</th>
                    <th width="120">操作</th>
                  </tr>
                </thead>
                <tbody>
